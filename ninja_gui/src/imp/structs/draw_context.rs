...
impl<'a, 'b, 'c> DrawContext<'a, 'b, 'c>{
    ...
    pub(crate) fn draw_text(&mut self, text : &str, color : GuiColor, font_size : u32,){
    ...
    pub(crate) fn fill_rect(&mut self, location : GuiPoint, size : GuiSize, color : GuiColor){
        rectangle(color.as_f32_array(),location.rect(size), self.context.transform.trans(100.,100.), self.g2d)
    }
}